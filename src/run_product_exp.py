    ...
    for i in range(EPOCHS):
        ...
        if test_acc > best_test_acc:
            ...
            mlflow.pytorch.log_model(
                model,
                artifact_path=f"{i}_{best_test_acc}"
            )
...
def create_experiment_and_run(params, learning_rates, dropouts, batch_sizes):
    experiment_name = f"batch_lr_dropout_effect"
    experiment_id = mlflow.get_experiment_by_name(experiment_name)
    if experiment_id is None:
        experiment_id = mlflow.create_experiment(experiment_name)
    else:
        experiment_id = experiment_id.experiment_id
    possible_values = list(product(batch_sizes, learning_rates, dropouts))
    for batch_size,lr, p in possible_values:
        run_name = f"{batch_size}_{lr}_{p}"
        ...
        with mlflow.start_run(
            ...
            acc = run_experiment(
                mlflow_client=mlflow_client, **params
            )
